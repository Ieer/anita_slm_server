...
set MODEL_DEVICE_MAP=auto          # transformers device_map，自訂 GPU 佈局
set ENABLE_TF32=1                  # 啟用 TF32（CUDA 需要 Ampere+）
set EMBEDDING_DEVICE=auto          # 嵌入模型裝置：auto/cpu/cuda:0
...
呼叫時在 Header 附帶：`X-API-Key: my-secret-key` 或 `Authorization: Bearer my-secret-key`。
...
- `/health/memory` 會額外回傳 `gpu_devices` 陣列，包含 GPU total/free/allocated。
- `/metrics` 暴露平均延遲、tokens/sec 與 GPU 記憶體 bytes 計量，方便 Prometheus 抓取。
